...
class Tree:
    ...
    def add(self, point1, point2):
        ...
        '''if point1 in self.neighbors_dict:
            self.neighbors_dict[point1].append(point2)
        else:
            self.neighbors_dict[point1] = [point2]

        self.neighbors_dict[point2] = []  # intialize an empty list for neighbors so ther are no errors when it is
        # called'''
    ...
    def rewire(self, point, r):
        ...
        if neighbors_list is None or len(neighbors_list) <= 0:
            return
        ...
        while replacement_node:  # to check if a new change was made, if there was then the loop is run again
            for node in neighbors_list:
                ...
                for temp_node in neighbors_list:
                    if temp_node == node or temp_node == self.start:
                        continue
                    if self.isCollisionFree(node, temp_node):  # only do the calculations if the points have a path
                        temp_node_cost = self.cost(temp_node)

                        dx = node[0] - temp_node[0]
                        dy = node[1] - temp_node[1]
                        angle_dist = self.angular_dist(node[2], temp_node[2])
                        temp_node_to_node = math.sqrt(pow(dx, 2) + pow(dy, 2) + pow(angle_dist, 2))
                ...
                if replacement_node:
                    ...
                    angle_dist = self.angular_dist(node[2], replacement_node[2])
                    temp_dist = math.sqrt(pow(dx, 2) + pow(dy, 2) + pow(angle_dist, 2))
                    self.node_data[node][0] = replacement_node
